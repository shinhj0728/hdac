...
// Originally written by Alexander Peslyak. Modified by WaterJuice retaining
...
//
static	void	*TransformFunction(Md5Context *ctx, const void *data, uintmax_t size)
{
    uint8_t*     ptr;
    uint32_t     a;
    uint32_t     b;
...
//
// Adds data to the MD5 context. This will process the data and update 
// the internal state of the context. Keep on calling this function until 
// all the data has been added. Then call Md5Finalise to calculate the hash.
//
void	Md5Update(Md5Context *Context, const void *Buffer, uint32_t BufferSize)
{
    uint32_t    saved_lo;
    uint32_t    used;
    uint32_t    free;
...
// (16 byte buffer containing 128bit hash). After calling this, 
// Md5Initialised must be used to reuse the context.
//
