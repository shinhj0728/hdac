...
//  Compress 1024-bits
//
static	void	TransformFunction(Sha512Context *Context, uint8_t *Buffer)
{
    uint64_t    S[8];
    uint64_t    W[80];
    uint64_t    t0;
...
// all the data has been added. Then call Sha512Finalise to calculate the hash.
//
void	Sha512Update(Sha512Context *Context, const void *Buffer, uint32_t BufferSize)
{
    uint32_t    n;
...
// (64 byte buffer containing 512bit hash). After calling this, 
// Sha512Initialised must be used to reuse the context.
//
